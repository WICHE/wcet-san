...
var _FilterItem = __webpack_require__(/*! ./search-main/components/FilterItem */ "./js/src/search-main/components/FilterItem.jsx");
 ...
 */
window.addEventListener('load', function () {
  var selects = document.querySelectorAll('.js-form-type-select');

  selects.forEach(function (select) {
    var label = select.querySelector('label');
    var dropDownTrigger = select.querySelector('.select2-selection');
  ...
  var select2s = document.querySelectorAll('.select2-selection');

  function operateAttributes(e) {
    var select2Element = e.target.closest('.select2-selection');

    if (select2Element && select2Element.getAttribute('aria-expanded') === 'true') {
      var selectElem = e.target.closest('.js-form-type-select');
      var labelID = selectElem.querySelector('label').getAttribute('id');
      var dropDownMenu = document.querySelector('.select2-results__options');
      var searchBar = document.querySelector('.select2-search--dropdown');
      var optionsNum = dropDownMenu.querySelectorAll('.select2-results__option').length;

      if (searchBar) {
        searchBar.querySelector('input').setAttribute('aria-labelledby', labelID);

        optionsNum > 15 ? searchBar.classList.add('searchable') : searchBar.classList.remove('searchable');
      }
    }
  }

  select2s.forEach(function (select) {
    select.addEventListener('click', function (e) {
      return operateAttributes(e);
    });
 ...
 */
window.addEventListener('load', function () {
  var copyButton = document.querySelector('.share-url-btn');

  if (copyButton) {
    var copyToClipboard = function copyToClipboard() {
      var copyText = copyButton.getAttribute('data-clipboard-text');
    ...
    copyButton.addEventListener('keyup', function (event) {
      if (event.key === 'Enter') {
        copyToClipboard();
      }
    });
...
/***/ "./js/src/search-main/components/FilterItem.jsx":
/*!******************************************************!*\
  !*** ./js/src/search-main/components/FilterItem.jsx ***!
  \******************************************************/
/*! no static exports found */
